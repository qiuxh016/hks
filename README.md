...
- **房间与剧本**：房主创建房间并选择剧本（2–6 人局，含 AI 补位）；其他玩家凭房间号或邀请链接加入；支持单人冒险模式。
- **准备机制**：大厅中真人玩家需点击"准备"；全部准备后房主方可开始游戏。
- **AI 主持人**：开局生成开场旁白、角色卡与世界目标；每轮根据玩家行动输出叙事，并维护地点、紧张度、任务等世界状态。
- **剧情记忆**：服务端维护故事摘要（走向 / 真相 / 目标），右侧面板实时展示。
- **回合制流程**：每轮先由真人玩家依次行动，再由 AI 机器人依次行动；支持快进跳过 AI 行动等待。
- **实时通信**：Socket.io 实时推送房间状态、投票事件、聊天消息和音频数据。
- **场景可视化**：CSS 艺术场景渲染 + 图片背景，可交互热点（搜查、观察等），支持对象聚焦特写（Close View），剧本专属视觉风格。
- **语音通话**：玩家之间按住按钮即可实时语音通话（MediaRecorder + Socket.io 音频中继）。
- **语音输入**：输入框右侧按住说话，语音实时转文字填入输入框。
- **投票系统**：每 3 回合自动触发投票，问题随剧本变化，30 秒截止或全员投票后公布结果。
- **玩家交流区**：故事流 / 交流区双标签，内置表情栏，未读消息徽章提示。
- **故事流滚动区**：Scene Focus 与输入框之间为可滚动消息区（带滚动条），逐条 typewriter 动画展示 AI 旁白和玩家行动，输入框和语音按钮固定在底部。
...
1. **大厅**：房主创建房间 → 选择剧本和人数 → 分享房间号 / 二维码 / 邀请链接 → 其他玩家加入并准备 → 全部准备后房主点击「开始游戏」。
2. **开局**：服务端为每位玩家分配角色卡（含背景、性格、隐藏目标）；不足人数用 AI 机器人补位；初始化交互场景；AI 主持人撰写开场旁白并设定故事走向与目标。
3. **进行中**：轮到你的回合时在底部输入框输入行动并提交；可通过场景面板点击交互热点快速操作；AI 主持人根据行动输出叙事；真人全部行动完毕后，AI 机器人自动依次行动（可快进跳过等待）。
...
```
┌──────────────────────────────────────────────────────┐
│ 左侧面板         │  中间 (故事流/交流区)  │ 右侧面板      │
│                  │                        │              │
│ 房间操作         │  Scene Focus            │ 任务目标      │
│ (创建/加入/开始) │  (场景 + 交互热点)       │ 故事走向      │
│                  │                        │ 系统日志      │
│ 场景选择         │  ┌ 滚动消息区 ┐         │ 玩家列表      │
│ 投票区           │  │ AI旁白     │         │ (角色/状态)   │
│                  │  │ 玩家行动   │         │              │
...
- **中间面板**：Scene Focus 底部为可滚动故事流（带滚动条），逐条展示 AI 主持人旁白和所有玩家的行动发言；输入框、语音按钮和执行按钮固定在底部，不随消息滚动。
- **右侧面板**：系统日志显示回合变更、玩家加入等通知；故事走向卡片显示当前剧情方向；玩家列表可展开查看角色详情。
- **左侧面板**：房间操作区，仅在未加入/大厅状态时显示完整操作项。
...
| `POST` | `/api/rooms/:roomId/fast-forward` | 快进 AI 机器人回合 |
